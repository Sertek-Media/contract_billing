...
class sale_advance_payment_inv(osv.osv_memory):
    _name = "sale.advance.payment.inv.task"
    ...
    def create_invoices(self, cr, uid, ids, context=None):
        ...
        if context.get('active_model',False) == 'project.task':
            sale_order_id = self.pool.get('project.task').read(cr,uid,context.get('active_ids',False),['sale_order_id'],context)
        
        if context.get('active_model',False) == 'project.issue':
            sale_order_id = self.pool.get('project.issue').read(cr,uid,context.get('active_ids',False),['sale_order_id'],context)
        
        list = []
        for i in sale_order_id:
            list.append(i.get('sale_order_id',False)[0])
        if list:
            sale_ids = [list[0]]    
        list = set(list)
        if len(list) != 1:
            raise osv.except_osv(
                        _('Error Invoicing  task!'),
                        _('Choose task with similar sales order'))
        sale_obj = self.pool.get('sale.order')
        act_window = self.pool.get('ir.actions.act_window')
        wizard = self.browse(cr, uid, ids[0], context)

        if context.get('active_model',False) == 'project.task':
            task_obj = self.pool.get('project.task').write(cr,uid,context.get('active_ids',False),{'is_fix_invoice':'yes'},context)
        
        if context.get('active_model',False) == 'project.issue':
            task_obj = self.pool.get('project.issue').write(cr,uid,context.get('active_ids',False),{'is_fix_invoice':'yes'},context)
